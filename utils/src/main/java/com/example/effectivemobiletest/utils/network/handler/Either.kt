package com.example.effectivemobiletest.utils.network.handler
...
inline fun <Error, Data> Either<Error, Data>.process(
    withData: (Data) -> Unit, withError: (Error) -> Unit
) {
    when (this) {
...
inline fun <Data, Error, DataOut, ErrorOut> Either<Error, Data>.map(
    success: (Data) -> DataOut, error: (Error) -> ErrorOut
...
inline fun <Data, Error, Result> Either<Error, Data>.unpack(
    success: (Data) -> Result, error: (Error) -> Result
): Result {
    return when (this) {
